...
- [ ] **Modifier vs mechanism disambiguation**: if token has `name=value` form where name is all-alphabetic AND not a known mechanism name, treat as modifier. Everything else parsed as directive. <!-- forge:cycle-1 -->
...
- [ ] `%{d0}` means all parts (0 = keep all, not zero parts). `%{d0r}` = split, reverse all, keep all. <!-- forge:cycle-1 -->
- [ ] `%{i}` IPv6 nibble output uses lowercase hex (consistent with RFC 7208 §7.3 examples) <!-- forge:cycle-1 -->
- [ ] `%{p}` expansion: return `"unknown"` (spec-allowed stub). Full PTR validation requires async DNS unavailable in pure expansion context. Evaluator can supply resolved value via `MacroContext` if desired. <!-- forge:cycle-1 -->
- [ ] `expand()` takes `exp_context: bool` flag — `true` only when expanding `exp=` TXT record content, `false` for domain-spec macros in mechanisms <!-- forge:cycle-1 -->
...
- [ ] **Add initial domain to visited set before starting evaluation** — prevents `redirect=same-domain` infinite loops <!-- forge:cycle-1 -->
...
- [ ] Expand macros in domain (macro expansion failure → `PermError`) <!-- forge:cycle-1 -->
- [ ] Recursively call `check_host()` with new domain
- [ ] Map child results:
  - [ ] `Pass` → match (use parent qualifier)
  - [ ] `Fail`, `SoftFail`, `Neutral`, `None` → no match (continue). Note: `None` means child domain has no SPF — this is NOT an error in the parent, just no match. <!-- forge:cycle-1 -->
...
- [ ] **Trait methods use `fn(...) -> impl Future<...> + Send`** (not `async fn in trait`). Native `async fn in trait` doesn't guarantee `Send` futures, which is required for `Pin<Box<dyn Future + Send>>` in recursive evaluation. <!-- forge:cycle-1 -->
- [ ] **Blanket impl `impl<R: DnsResolver> DnsResolver for &R`**: enables passing `&self.resolver` to sub-verifiers. Use UFCS `<R as DnsResolver>::method(self, ...)` to avoid infinite recursion. <!-- forge:cycle-1 -->
- [ ] Methods needed:
...
- [ ] MX host DNS error → skip that host, continue (partial MX resolution failure test) <!-- forge:cycle-1 -->
...
- **CRITICAL BUG in v2**: `eval_a_mechanism` and `eval_ptr_mechanism` used `let _ = ctx.increment_void()` which silently discards the PermError when limit exceeded
- **FIX**: These functions must propagate the void limit error. Change return type to `Result<bool, SpfResult>` and use `?` operator, or check the result explicitly
- Same pattern applies to `eval_mx_mechanism` internal void tracking on NxDomain MX responses

### 9.5 TempFail Handling in A/MX Mechanisms
- v2 treats DNS TempFail in A mechanism as "no match" (returns false)
- RFC 7208 Section 4.6.4 says this should be TempError in strict interpretation
- Current behavior is lenient — acceptable for production robustness but not spec-strict
- Consider: return `Err(SpfResult::TempError)` on DNS TempFail in A/AAAA queries within mechanisms
...
### 9.8 v4 (Cycle 1) Learnings <!-- forge:cycle-1 -->

#### 9.8.0 DnsResolver Trait Design
- `async fn in trait` doesn't guarantee `Send` futures → must use `fn(...) -> impl Future<...> + Send` on trait methods
- Propagated through blanket `impl DnsResolver for &R` and MockResolver
- `#[allow(async_fn_in_trait)]` only viable if no `dyn DnsResolver` or `Pin<Box<dyn Future + Send>>` usage

#### 9.8.0b PTR Mechanism DNS Counting
- PTR forward-confirmation lookups are NOT counted toward the 10-DNS-lookup limit
- RFC 7208 is ambiguous — the PTR mechanism itself counts as 1, forward confirmations are internal
- This matches common implementations (postfix-policyd, libspf2)

#### 9.8.0c Void Lookup Propagation
- `increment_void()` returns `Result` — MUST propagate via `?` operator
- `let _ = ctx.increment_void()` silently discards PermError when limit exceeded
- Apply in all mechanism evaluators: `eval_a`, `eval_mx`, `eval_ptr`

### 9.8 (archived) v3 Learnings

#### 9.8.1 Test Coverage Gaps
- **Duplicate redirect/exp detection**: spec §2.2 requires PermError for `redirect=a redirect=b`. Parsing enforced this but no dedicated test existed — add explicit test.
- **CIDR prefix edge cases**: `a/0` and `a//0` (match-all) need dedicated tests to verify they match any IP, not just "no match".
- **%{c}/%{r}/%{t} value correctness**: Tests verified these macros are REJECTED in non-exp context, but never tested that `%{c}` produces the correct client IP string, `%{r}` produces the receiver hostname, and `%{t}` produces a valid timestamp in exp= context. Add ground-truth tests.

#### 9.8.2 Patterns That Worked
- `EvalContext` as `&mut` shared state through async recursion — clean and correct
- Macro expansion as standalone module with clear context struct — easy to test independently
- Mechanism-level evaluation functions returning `Result<bool, SpfResult>` — cleanly propagates errors vs match/no-match
