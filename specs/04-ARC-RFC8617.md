...
- `ArcSealer::seal_message()` returns 3 header values (AAR, AMS, AS) — caller prepends header names
- Sealer needs access to existing ARC chain for cv= determination — pass headers/body
- Sealer generates AMS using DKIM signing primitives, then AS over all ARC headers
- The sealer does NOT need a DnsResolver — it only signs, never verifies (validation is caller's choice)
